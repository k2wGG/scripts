  ...
  case "$LANG_CHOICE" in
    en)
      case "$k" in
        ...
        m17_redeploy_trap) echo "Redeploy of Trap";;
    ...
    *)
      case "$k" in
        ...
        m17_redeploy_trap) echo "Сделать Redeploy of Trap";;
...
update_node_safe() {
  ...
  # сначала обновляем через droseraup, если есть
  if command -v droseraup >/dev/null 2>&1; then
    droseraup || true
    home_ver=$(get_operator_version_of "$HOME_BIN" || echo "$home_ver")
  fi

  # если в ~/.drosera/bin уже лежит latest – просто синкаем в /usr/bin,
  # иначе качаем релиз с GitHub
  if [[ -n "$latest" && "$home_ver" == "$latest" ]]; then
    sync_operator_bin || true
  else
    update_operator_bin || true
  fi

  usr_ver=$(get_operator_version_of "$USR_BIN" || echo "")
  if [[ -n "$latest" && "$usr_ver" != "$latest" && -x "$HOME_BIN" ]]; then
    sudo install -m 0755 "$HOME_BIN" "$USR_BIN"
    usr_ver=$(get_operator_version_of "$USR_BIN" || echo "")
  fi

  # --- Миграция systemd-юнита с --db-file-path на --data-dir ---
  if [[ -f "$SERVICE_FILE" ]]; then
    if grep -q -- '--db-file-path' "$SERVICE_FILE"; then
      info "Обновляю юнит: меняю --db-file-path на --data-dir ($DATA_DIR)..."
      sudo mkdir -p "$DATA_DIR"
      sudo sed -E -i "s|--db-file-path[[:space:]]+[^[:space:]]+|--data-dir $DATA_DIR|g" "$SERVICE_FILE"
    fi
  fi
  ...
  if [[ -n "$run_pid" ]]; then
    run_path=$(readlink -f "/proc/$run_pid/exe" 2>/dev/null || true)
    run_ver=$(get_operator_version_of "$run_path")
  fi

  if [[ "$status" == "active" ]]; then
    ok "$(tr service_active). $(tr running_ver): ${run_ver:-unknown}"
  else
    err "$(tr service_inactive)"
  fi
}
